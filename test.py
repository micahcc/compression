...
parser.add_argument("-n", "--name", default="", help="output training details")
parser.add_argument("-p", "--pretrain", default="", help="load pretrain model")
parser.add_argument("-t", "--test", default="", help="test dataset")
parser.add_argument(
...
parser.add_argument(
    "--val", dest="val_path", required=True, help="the path of validation dataset"
)
...
def test(step):
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ...
            cnt += 1
            logger.info(
                "Num: {}, Bpp:{:.6f}, PSNR:{:.6f}, MS-SSIM:{:.6f}, MS-SSIM-DB:{:.6f}".format(
        ...
        sumMsssimDB /= cnt
        logger.info(
            f"Dataset Average result---Dataset Num: {cnt}, Bpp:{sumBpp}, "
            f"PSNR:{sumPsnr}, MS-SSIM:{sumMsssim}, MS-SSIM-DB:{sumMsssimDB}"
        )
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(
        dataset=test_dataset, shuffle=False, batch_size=1, pin_memory=True
    )
    test(global_step)
