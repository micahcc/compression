...
from models.analysis import Analysis_net
from models.synthesis import Synthesis_net
...
class ImageCompressor(nn.Module):
    ...
    def forward(self, input_image):
        ...
        quant_noise_z = torch.zeros(
            input_image.size(0),
            self.out_channel_N,
            input_image.size(2) // 64,
            input_image.size(3) // 64,
        ).cuda()
        quant_noise_feature = torch.nn.init.uniform_(
            torch.zeros_like(quant_noise_feature), -0.5, 0.5
        )
        quant_noise_z = torch.nn.init.uniform_(
            torch.zeros_like(quant_noise_z), -0.5, 0.5
        )
        ...
        total_bits_feature, _ = feature_probs_based_sigma(
